...
const ComposeMessageModal = ({ isOpen, onClose, onSent, initialRecipient = null }) => {
  ...
  useEffect(() => {
    if (initialRecipient) {
      setFormData(prev => ({
        ...prev,
        recipientType: initialRecipient.type || 'user',
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!formData.recipientId || !formData.messageText) {
        ...
        <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
          <form onSubmit={handleSubmit}>
            ...
            <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
              ...
              <div className="mb-4">
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  {formData.recipientType === 'user' ? 'User ID' : 
                   formData.recipientType === 'team' ? 'Team/Project ID' : 'Project ID'}
                </label>
                <input
                  type="text"
                  name="recipientId"
                  ...
                  onChange={handleInputChange}
                  placeholder={`Enter ${formData.recipientType} ID`}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                  required
                />
              </div>
